class golang ( $version = "1.4.1" ) {
  ...
  exec { "setup-path":
    command => "/bin/echo 'export PATH=/vagrant/bin:/usr/local/go/bin:\$PATH' >> /home/vagrant/.bash_profile",
    unless  => "/bin/grep -q /usr/local/go /home/vagrant/.bash_profile ; /usr/bin/test $? -eq 0"
  }
